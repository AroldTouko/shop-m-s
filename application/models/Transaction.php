...
class Transaction extends CI_Model {
    ...
    public function getAll($orderBy, $orderFormat, $start, $limit) {
        if ($this->db->platform() == "sqlite3") {
            ...
            if($_SESSION['admin_role'] == "Super"){
                ...
                $this->db->join('admin', 'transactions.staffId = admin.id', 'INNER');
            }
            
            //ligne ajoutée
            if($_SESSION['admin_role'] !== "Super"){
                $this->db->where("transactions.staffId",$_SESSION['admin_id']);
            }
    ...
    public function transSearch($value) {
        ...
        $this->db->or_like('cust_name', $value);
        $this->db->or_like('cust_phone', $value);
        $this->db->or_like('cust_email', $value);
        $this->db->group_by('ref');
    ...
    public function getDateRange($from_date, $to_date){
        if ($this->db->platform() == "sqlite3") {
            ...
            $this->db->where("DATE_FORMAT(transactions.transDate, '%Y-%m-%d %H:%i') >= ", "".strval($from_date));
            $this->db->where("DATE_FORMAT(transactions.transDate, '%Y-%m-%d %H:%i') <= ", "".strval($to_date));
