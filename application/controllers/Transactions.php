...
class Transactions extends CI_Controller{
    ...
    */
    ...
    private function genTransReceipt($allTransInfo, $cumAmount, $_at, $_cd, $ref, $transDate, $_mop, $vatAmount, $vatPercentage, 
        ...
        foreach($allTransInfo as $trans){
			$staffId = isset($trans['staffId']) ? $trans['staffId']: $_SESSION['admin_id'];
		}
		$staffs = $this->admin->get_admin_info_by_id($staffId);
    ...
    */
    ...
    public function report($from_date, $to_date=''){        
        ...
        $from =  str_replace('T',' ',$from_date);
        $to = str_replace('T',' ',$to_date);
        $data['from'] = $from;
        $data['to'] = $to ? $to : date('Y-m-d');
        
        $data['allTransactions'] = $this->transaction->getDateRange($from, $to);
